...
void IMU_read() {
    //Going to read all 7 values (temp, position, and angle)
    //Values are 16 bytes. Grab them as 8 bytes and then concatenate them
    ...
    for (i = 0;i < 14; i++) {
        if (i%2 == 0) { // if 0, LOW data
            master_read_L = i2c_master_recv();
            i2c_master_ack(0); //send bytes continuously
        }
        else { // HIGH data
            ...
            if (i == 13) {
                ...
                i2c_master_ack(0); // send data continuously
            }
        }
    }
    
    i2c_master_stop(); //array of shorts
    
    LCD_print_IMU_bars(data_array[4]*0.0061, data_array[5]*0.0061); //draw bars
